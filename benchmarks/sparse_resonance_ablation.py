...
def render_markdown(results: Dict) -> str:
    ...
    for row in results["runs"]:
        ...
        lines.append(
            ...
            f"{row['elapsed_s']:.4f} | {row['tokens_per_s']:.1f} | "
...
def main():
    ...
    dense_logits, dense_time = _elapsed(device, lambda: run_full(dense_model, tokens))
        ...
        "runs": [
            {
                ...
                "elapsed_s": dense_time,
                "tokens_per_s": args.steps / max(dense_time, 1e-9),
    ...
    for top_k in args.top_k:
        ...
        sparse_logits, sparse_time = _elapsed(device, lambda: run_full(sparse_model, tokens))
        sparse_diag = gate_diagnostics(sparse_model)
        results["runs"].append({
            "mode": "sparse",
            "top_k": top_k,
            "elapsed_s": sparse_time,
            "tokens_per_s": args.steps / max(sparse_time, 1e-9),
