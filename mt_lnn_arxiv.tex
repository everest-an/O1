...
We evaluated MT-LNN as a residual adapter on the \texttt{TinyLlama-1.1B} architecture (fine-tuned for 500 steps). In a standard Needle-in-a-Haystack test, the MT-Adapter achieved 100\% retrieval accuracy across all depths (10\%--90\%) at context lengths up to 2048 (the native limit of the base model). Inference throughput degraded by only $\sim\% (from $\sim Tok/s to $\sim Tok/s), demonstrating that the biological continuous-time dynamics can smoothly integrate with standard causal LLMs without disrupting zero-shot instruction following or reasoning capabilities.
