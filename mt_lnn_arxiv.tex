...
\begin{table}[h]
\centering
\caption{Selective Copy: held-out sequence-exact accuracy (\%), 5-seed mean $\pm$ std, with the corrected evaluation harness. At matched compute the three architectures are broadly comparable; MT-LNN does \emph{not} pull ahead as $T$ grows.}
\label{tab:long_ctx}
\begin{tabular}{lcccc}
\toprule
...
\textbf{Correction and honest finding.} An earlier version of this paper reported a ``growing advantage'' ($\times$17 $\to$ $\times$27 $\to$ $\times$6) for MT-LNN. That result was an \emph{evaluation artefact}: the baseline models had no KV-cache implementation, so the incremental greedy decoder fed them a single token with no context for targets $2\!-\!K$, collapsing them to blind guessing (their old scores match the theoretical floor $(1{+}(K{-}1)/k)/K$ exactly). With the corrected harness (full-sequence recompute for cache-less models) and 5 seeds, the ``$\times42$'' headline disappears: MT-LNN is at best tied at $T=37$ (within overlapping seed variance) and is \emph{beaten} by both plain baselines at $T=101$ and $T=229$, where its training becomes unstable (seq-exact $3.9\% \pm 4.9\%$ at $T=229$, near the random floor). We report this negative result faithfully. On this task softmax attention is expected to be near-perfect regardless \citep{gu2023mamba}; Selective Copy separates LTI from \emph{selective} SSMs, and is not a task on which a recurrent model should beat attention.
...
An earlier version of this appendix reported a grid of $\hat{\Phi}$-collapse percentages (71--96\%) claiming MT-LNN passes the anesthesia test across all hook-sensitivity settings. Those numbers were not reproduced by the released code and contradict the actual toy-scale result (Section~\ref{sec:anesth_results}), where $\hat{\Phi}$ \emph{rises} with $\kappa$ (collapse $0\%$). They are withdrawn. A meaningful sensitivity sweep requires a model trained at scale to a positive baseline integration; it is listed as future work.
