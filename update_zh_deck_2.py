...
\end{columns}
\end{frame}"""
text = inject(r'\\begin\{frame\}\{我们的方案：MT-LNN 架构登场\}[\s\S]*?\\end\{frame\}', new_f1, text)
...
\end{columns}
\end{frame}"""
text = inject(r'\\begin\{frame\}\{超越“文字接龙”：真正理解真实物理世界规律\}[\s\S]*?\\end\{frame\}', new_f2, text)
...
\end{itemize}
\end{frame}"""
text = inject(r'\\begin\{frame\}\{深度解析：并行扫描与量子耦合\}[\s\S]*?\\end\{frame\}', new_f3, text)
