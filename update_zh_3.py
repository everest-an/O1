...
abstract_end = content.find('\\end{abstract}') + 14

import sys
if abstract_start == -1 or abstract_end < 14:
    sys.exit(1)
...
contrib_start = content.find('\\medskip\\noindent\\textbf{主要贡献。}')
contrib_end = content.find('\\end{enumerate}', contrib_start) + 15
...
if contrib_start != -1 and contrib_end >= 15:
    content = content[:contrib_start] + new_contrib + content[contrib_end:]
