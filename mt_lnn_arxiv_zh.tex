...
在 1.1B 参数规模下，我们将 MT-LNN 作为残差适配器 (Residual Adapter) 挂载到 TinyLlama-1.1B 模型上进行 500 步微调。大海捞针测试结果显示：在 1024 和 2048 长度下，带有微管适配器的模型在各个深度 (10\%~90\%) 均保持了 100\% 的准确率，完全没有破坏基础模型原有的指令跟随能力。相比基础模型 (~800 Tok/s)，挂载适配器后的推理速度仅下降约 13\% (~670 Tok/s)，证明了该架构在保持极高运行效率的同时，能够完美兼容主流大语言模型原生的大海捞针长下文检索能力。(注：4096 长度因超出底座原生 2048 窗口而失效)。
