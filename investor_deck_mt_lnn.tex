...
\begin{itemize}
    ...
    \item \textbf{Core Philosophy:} The Edge \textbf{M1 Engine} serves as the continuous "soul" and reasoning engine, while our proprietary \textbf{Awareness Cloud} (High-Param MT-LNN) acts as the global knowledge Oracle.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{Local Brain (Edge M1):} 90\% of logical reasoning, continuous memory, and private deliberation are handled purely on-device via a lightweight MT-LNN, achieving \textbf{Zero-Cost Local Compute}.
        \item \textbf{Cloud Oracle (Awareness Cloud):} The cloud is no longer a high-frequency token black hole. It serves simply as a heavy-parameter knowledge base.
    \end{itemize}
    \vspace{0.4em}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item In factual blind spots, M1 dispatches surgical queries to our proprietary \textbf{Awareness Cloud}. Legacy APIs are excluded; instead, M1 fluidly synthesizes world knowledge through its personalized state via a unified MT-LNN ecosystem.
    \end{itemize}
\end{itemize}
\end{frame}
...
\begin{frame}{Verifiable Benchmarks \& Open Reproducibility}
\begin{itemize}
    \item \textbf{1. Memory Compression Evidence (1000-token footprint)}
    \begin{itemize}
        \item \textbf{Data:} Transformer KV = 1020 KB v.s. MT-LNN = 4.1 KB.
        \item \textbf{Meaning:} Enables continuous "always-on" AI on strictly limited edge devices without RAM bottlenecks (OOM).
        \item \textbf{Reproduce Action:} Run \texttt{python benchmarks/operator\_compression\_report.py}.
    \end{itemize}
    \vspace{0.4em}
    \item \textbf{2. Long-Context Accuracy Evidence ("Needle in a Haystack")}
    \begin{itemize}
        \item \textbf{Data:} In 200K parameter head-to-head, Transformer retrieval crashes to 2.3\% while MT-LNN sustains 96.5\% (a 42x advantage).
        \item \textbf{Meaning:} Absolute fidelity for tracking user history, inherently suppressing hallucinations.
        \item \textbf{Reproduce Action:} Run \texttt{python benchmarks/run\_benchmark.py}.
    \end{itemize}
    \vspace{0.4em}
    \item \textbf{3. CPU Speed Evidence (Sparse Resonance)}
    \begin{itemize}
        \item \textbf{Data:} Dense processing at 6528 Tok/s jumps to 7390 Tok/s.
        \item \textbf{Meaning:} Biologically-inspired "sleep" patterns skip redundant compute, slashing inference bills.
        \item \textbf{Reproduce Action:} Run \texttt{python benchmarks/sparse\_resonance\_ablation.py}.
    \end{itemize}
\end{itemize}
\end{frame}
